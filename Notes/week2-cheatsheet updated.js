...
// let colors = ["Blue", "Black", "Red", "White"];
// console.log(colors[0], colors[colors.length -1]);
// colors[1] = "Green";
// let nestedArray = ["Yellow","Purple","Pink"]
...
// let height = 180;//global
// {
//   let weight = 70;//block scope 1
//   {//block scope 2
...
// if(userName === "User 123"){// false || true
...
// //colors[1] = blue
// //colors[2] = green
// while(i < colors.length ){// colors.length = 3, 0 -> 1 -> 2 -> 
//   console.log(colors[i]);//colors[0] --> colors[1] --> colors[2]
//   i++
// };
...
// ];
// let bigCities = []
// // //1 city = { name: "New York", population: 18.65e6 } --> console.log(city["name"])/ console.log(city.population)
// // //2 city  = { name: "Cairo", population: 18.82e6 }  ---> console.log(city.name) / console.log(city.name) 
// // //3 city = { name: "Mumbai", population: 19.32e6 }  ---> console.log(city.name) / console.log(city.name) 
// // //4 city = { name: "São Paulo", population: 20.88e6 }  ---> console.log(city.name) / console.log(city.name) 
...
/* =========================
  ...
  // splice(startIndex, deleteCount, ...items): in-place edit --> changes the original array
  names.splice(1, 3, "Paul", "James"); // replace 3 items starting at index 1
  console.log("after splice →", names);

  // slice(startIndex, endExclusiveIndex): returns new array (non-destructive)
